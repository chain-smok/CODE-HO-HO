...
{
   i=visitedI.front(); visitedI.pop();
   j=visitedJ.front(); visitedJ.pop();
   steps=visitedM.front();visitedM.pop();
}
bool inRange(int i, int j, int n)
{
   if(i<0 || j<0 || i>=R || j>=C  
         || step[n%3][i][j]>-1 
         || map[i][j]=='#'){
      return false;//visited or a wall
   }else return true;
}
